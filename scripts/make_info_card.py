...
ROWS = [
    ("host",),
    ("kv", "Role", "Data Analyst"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Languages", "Python, C"),
    ("kv", "Frontend", "React"),
    ("kv", "Databases", "SQL Server"),
    ("kv", "Tools/Cloud", "Git, Azure"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Building and contributing to awesome projects"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">joel1fps@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">joel1fps</tspan><tspan fill="{MUTED}">@</tspan>'
